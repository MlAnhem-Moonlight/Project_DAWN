...
public class DefensiveAction : Nodes
{
    ...
    private void SwitchAnim(Vector3 targetPos, string state1, string state2,float crossFade = 0.1f)
    {
        if (_transform.position.x - targetPos.x > 0) _controller.ChangeAnimation(_anim, state1, crossFade);
        else _controller.ChangeAnimation(_anim, state2, crossFade);
    }
}
