...
public class ResourceAllocationGA : MonoBehaviour
{
    ...
    // Fix for CS0246: Replace 'Chromosome' with 'Individual' in the Crossover method signature and return type
        ...
        if (random.NextDouble() < crossoverRate)
        {
            ...
            switch (random.Next(3))
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    ArithmeticCrossover(child1, child2);
                    break;
            }
        }
    ...
    private void UniformCrossover(Individual child1, Individual child2)
    {
        // Existing uniform crossover logic
    ...
    {
        int crossoverPoint = random.Next(child1.allocations.Count);
        for (int i = crossoverPoint; i < child1.allocations.Count; i++)
        {
            var temp = child1.allocations[i].quantity;
            child1.allocations[i].quantity = child2.allocations[i].quantity;
            child2.allocations[i].quantity = temp;
    ...
    private void ArithmeticCrossover(Individual child1, Individual child2)
    {
        float alpha = (float)random.NextDouble(); // Mixing ratio
    
        for (int i = 0; i < child1.allocations.Count; i++)
        {
            int q1 = child1.allocations[i].quantity;
            int q2 = child2.allocations[i].quantity;
        
            child1.allocations[i].quantity = (int)(alpha * q1 + (1 - alpha) * q2);
            child2.allocations[i].quantity = (int)(alpha * q2 + (1 - alpha) * q1);
        }
    }

    // Modify the Mutation method to include multiple mutation types
    private void Mutate(Individual individual)
    {
        if (random.NextDouble() < mutationRate)
        {
            ...
            {
                case 0:
                    SinglePointMutation(individual);
                    break;
                case 1:
    ...
    private void SinglePointMutation(Individual individual)
    {
        // Existing mutation logic
        int index = random.Next(individual.allocations.Count);
        var allocation = individual.allocations[index];
        var remaining = CalculateRemainingResources(individual, allocation);
        int maxQuantity = CalculateMaxQuantity(allocation.resourceCost, remaining);
    ...
    {
        if (individual.allocations.Count < 2) return;
    
        int idx1 = random.Next(individual.allocations.Count);
        int idx2 = random.Next(individual.allocations.Count);
    
        while (idx2 == idx1)
            idx2 = random.Next(individual.allocations.Count);
        
        var temp = individual.allocations[idx1].quantity;
        individual.allocations[idx1].quantity = individual.allocations[idx2].quantity;
        individual.allocations[idx2].quantity = temp;
    }
    ...
    private void GaussianMutation(Individual individual)
    {
        ...
        double mean = allocation.quantity;
        double stdDev = Math.Max(1.0, mean * 0.1); // 10% of current value
        double gaussian = NextGaussian() * stdDev + mean;
    
        // Ensure valid bounds
        var remaining = CalculateRemainingResources(individual, allocation);
        int maxQuantity = CalculateMaxQuantity(allocation.resourceCost, remaining);
        allocation.quantity = (int)Math.Max(0, Math.Min(maxQuantity, gaussian));
    }

    private void ScrambleMutation(Individual individual)
    {
        // Randomly scramble a subset of allocations
        int start = random.Next(individual.allocations.Count);
        int length = random.Next(1, individual.allocations.Count - start + 1);
    
        var range = Enumerable.Range(start, length).ToList();
        for (int i = range.Count - 1; i > 0; i--)
        {
            int j = random.Next(i + 1);
            var temp = individual.allocations[range[i]].quantity;
            individual.allocations[range[i]].quantity = individual.allocations[range[j]].quantity;
            individual.allocations[range[j]].quantity = temp;
        }
    }

    // Helper method for Gaussian mutation
    ...
    // Helper method to calculate remaining resources
    private ResourceDataGA CalculateRemainingResources(Individual individual, GameObjectAllocation excludeAllocation)
    {
        var used = individual.GetTotalUsedResources();
        return new ResourceDataGA(
            availableResources.wood - used.wood + excludeAllocation.GetTotalCost().wood,
            availableResources.stone - used.stone + excludeAllocation.GetTotalCost().stone,
            availableResources.iron - used.iron + excludeAllocation.GetTotalCost().iron,
            availableResources.gold - used.gold + excludeAllocation.GetTotalCost().gold,
            availableResources.meat - used.meat + excludeAllocation.GetTotalCost().meat
        );
    }
